# VIE-SPEC-REP 1.1.0 — Intensity Objects (additive revision of 1.0.0)

- Document version: **1.1.0** · Supersedes: `VIE-SPEC-REP` 1.0.0 **additively**
- Status: normative from Phase 2 onward (approved by DEC-0003)
...
3. **Empty case.** A monotone frame cannot produce zero objects: every pixel belongs to
   exactly one component of its own level. The count of objects can be zero only if the
   frame itself is empty, which §2.4 of 1.0.0 forbids (min 1×1 still yields 1 object).
...
## R10. Known limitations of 1.1.0
